...
#define i2c_write(a, b, c, d) stm32_spi_write(a, b, c, d)
#define i2c_read(a, b, c, d)  stm32_spi_read(a, b, c, d)
...
static int setup_compass(void)
{
    ...
    st.chip_cfg.compass_addr = 0x0C;//akm_addr;
    ...
    user_ctrl |= 0x10; //Disable I2C Slave module and put the serial interface in SPI mode only
    user_ctrl |= 0x20; //Enable the I2C Master I/F module;

    if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, &user_ctrl))
        return -1;

    if (i2c_read(st.chip_cfg.compass_addr, AKM_REG_WHOAMI, 1, data) || (data[0] != AKM_WHOAMI)) {
    	log_e("Compass not found WHOAMI: 0x%x .\r\n", data[0]);
		return -1;
    }
