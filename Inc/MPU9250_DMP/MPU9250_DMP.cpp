...
inv_error_t MPU9250_DMP::begin(void)
{
	...
	mpu_set_bypass(0); // Place all slaves (including compass) on primary bus
...
void MPU9250_DMP::computeEulerAngles(bool degrees)
{
	...
	float sinr = +2.0 * (dqw * dqx + dqy * dqz);
	float cosr = +1.0 - 2.0 * (dqx * dqx + dqy * dqy);
	roll = atan2(sinr, cosr);

	// pitch (y-axis rotation)
	float sinp = +2.0 * (dqw * dqy - dqz * dqx);
	if (fabs(sinp) >= 1)
	...
	// yaw (z-axis rotation)
	float siny = +2.0 * (dqw * dqz + dqx * dqy);
	float cosy = +1.0 - 2.0 * (dqy * dqy + dqz * dqz);
	yaw = atan2(siny, cosy);
